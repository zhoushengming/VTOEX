package com.ericcode.vtoex.extention
...
var TextView.textGone: CharSequence?
    set(value) {
        if (value == null) {
...
var TextView.textInVisible: CharSequence?
    set(value) {
        if (value == null) {
...
val Long.lossTimeCN: String?
    get() {
        return Date(this).lossTimeCN
    }

val Date.lossTimeCN: String?
    get() {
        return RelativeDateFormat.format(this)
    }
...
object RelativeDateFormat {
    ...
    fun formatSplit(date: Date): Array<String> {
        val delta = Date().time - date.time
        if (delta < 1L * ONE_MINUTE) {
            val seconds = toSeconds(delta)
            return arrayOf((if (seconds <= 0) 1 else seconds).toString() + "", ONE_SECOND_AGO)
        }
        if (delta < 45L * ONE_MINUTE) {
            val minutes = toMinutes(delta)
            return arrayOf((if (minutes <= 0) 1 else minutes).toString() + "", ONE_MINUTE_AGO)
        }
        if (delta < 24L * ONE_HOUR) {
            val hours = toHours(delta)
            return arrayOf((if (hours <= 0) 1 else hours).toString() + "", ONE_HOUR_AGO)
        }
        if (delta < 48L * ONE_HOUR) {
            return arrayOf("昨天")
        }
        if (delta < 30L * ONE_DAY) {
            val days = toDays(delta)
            return arrayOf((if (days <= 0) 1 else days).toString() + "", ONE_DAY_AGO)
        }
        if (delta < 12L * 4L * ONE_WEEK) {
            val months = toMonths(delta)
            return arrayOf((if (months <= 0) 1 else months).toString() + "", ONE_MONTH_AGO)
        } else {
            val years = toYears(delta)
            return arrayOf((if (years <= 0) 1 else years).toString() + "", ONE_YEAR_AGO)
        }
    }
